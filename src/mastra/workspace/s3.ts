...
import type { FileEntry, ReadOptions, WriteOptions, ListOptions, RemoveOptions, CopyOptions, FileContent } from "@mastra/core/workspace";
...
class PatchedS3Filesystem extends S3Filesystem {
  ...
  override writeFile(path: string, content: FileContent, options?: WriteOptions) {
  ...
  override copyFile(src: string, dest: string, options?: CopyOptions) {
    return super.copyFile(this.normalizePath(src), this.normalizePath(dest), options);
  }

  override moveFile(src: string, dest: string, options?: CopyOptions) {
    return super.moveFile(this.normalizePath(src), this.normalizePath(dest), options);
  }
