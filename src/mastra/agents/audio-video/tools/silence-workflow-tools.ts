...
Supported formats: mp4, mov, avi, mp3, m4a, wav, ogg, flac`,
  inputSchema: z.object({
    file: z
      .string()
      .describe(
        "Relative path within the workspace (e.g., podcast.m4a, wild_project.mp4)"
      ),
  ...
  outputSchema: z.object({
    ...
    runId: z
      .string()
      .describe("Workflow run ID — pass to resume-silence-cutter"),
    summary: z.string(),
    segments: z.array(
      z.object({
        start: z.number(),
  ...
  execute: async (inputData, context: ToolExecutionContext) => {
    ...
    if (!mastra) {
      console.error(
        "[startSilenceCutterTool] ERROR: Mastra instance not available"
      )
    ...
    if (!workflow) {
      ...
      return {
        ...
        segments: [],
        message:
          "Workflow not found. Verify that silenceCutterWorkflow is registered.",
    ...
    console.log(
      "[startSilenceCutterTool] Result of run.start():",
    ...
    console.log(
      "[startSilenceCutterTool] Workflow did NOT suspend. Status:",
      result.status
    )
    return {
      ...
      segments: [],
      message:
        "The workflow did not reach the approval checkpoint. Verify the file exists.",
      error:
        "The workflow did not reach the approval checkpoint. Verify the file exists.",
    }
  },
...
Always pass the runId returned by start-silence-cutter when available.`,
  inputSchema: z.object({
    runId: z
      ...
      .optional()
      .describe(
        "Run ID returned by start-silence-cutter. If not provided, searches for the last suspended run."
      ),
  ...
  execute: async (inputData, context: ToolExecutionContext) => {
    ...
    if (!mastra) {
      console.error(
        "[resumeSilenceCutterTool] ERROR: Mastra instance not available"
      )
      throw new Error("Mastra instance not available in tool context")
    }
    ...
    const workflow = mastra.getWorkflow("silenceCutterWorkflow")
    console.log(
      "[resumeSilenceCutterTool] Workflow found:",
      workflow ? "YES" : "NO"
    )
    ...
    if (providedRunId) {
      console.log(
        "[resumeSilenceCutterTool] Using provided runId:",
        providedRunId
      )
      runId = providedRunId
    } else {
      ...
      console.log(
        "[resumeSilenceCutterTool] No runId provided, searching for suspended runs..."
      )
      ...
      console.log(
        "[resumeSilenceCutterTool] Runs found:",
        runs.runs?.length || 0
      )

      if (!runs.runs || runs.runs.length === 0) {
        console.error(
          "[resumeSilenceCutterTool] ERROR: No suspended runs found"
        )
        return {
          skipped: true,
          message:
            "No pending silence detection found. First analyze a file with start-silence-cutter.",
          status: "error",
      ...
      if (!suspendedRun) {
        console.error(
          "[resumeSilenceCutterTool] ERROR: Suspended run is undefined"
        )
        return {
          skipped: true,
    ...
    console.log(
      "[resumeSilenceCutterTool] Result of run.resume():",
      JSON.stringify(result, null, 2)
    )
    ...
    console.log(
      "[resumeSilenceCutterTool] Workflow finished with status:",
      result.status
    )
    return {
      skipped: false,
