...
export const subtitleStylePresetSchema = z.enum([
  "shorts-bold",
  "minimal-clean",
  "cinema-pop",
  "viral-neon",
...
const DEFAULT_STYLE_PRESET: z.infer<typeof subtitleStylePresetSchema> =
  "shorts-bold"
...
function resolveDynamicStyleMetrics(
  ...
  const fontSize = clamp(
    Math.round(videoHeight * presetConfig.relativeSize * ratioBoost),
    24,
    78
  )
  const marginV = clamp(
    Math.round(videoHeight * (isVertical ? 0.075 : 0.055)),
    24,
    120
  )
...
function buildSubtitleForceStyle(
  ...
  const outline =
    preset === "viral-neon" ? clamp(metrics.outline + 1, 2, 7) : metrics.outline
  const shadow =
    preset === "viral-neon" ? clamp(metrics.shadow + 1, 0, 3) : metrics.shadow
...
export const subtitleBurnApprovalStep = createStep({
  ...
  resumeSchema: z.object({
    ...
    textCase: subtitleTextCaseSchema
      .optional()
      .default(DEFAULT_SUBTITLE_TEXT_CASE),
    layoutMode: subtitleLayoutModeSchema
      .optional()
      .default(DEFAULT_SUBTITLE_LAYOUT_MODE),
    animationPreset: subtitleAnimationPresetSchema
      .optional()
      .default(DEFAULT_SUBTITLE_ANIMATION_PRESET),
    safeAreaBottomPercent: z
      .number()
      .optional()
      .default(DEFAULT_SAFE_AREA_BOTTOM_PERCENT),
  }),
  outputSchema: baseSubtitleInputSchema.extend({
    ...
    layoutMode: subtitleLayoutModeSchema,
    animationPreset: subtitleAnimationPresetSchema,
    safeAreaBottomPercent: z.number(),
  ...
  execute: async ({ inputData, requestContext, resumeData, suspend }) => {
    ...
    try {
      const sourceFilePath = inputData.sourceFilePath

      // Download source file to check if it's video
      const { localPath: sourceAbsPath } = await ensureLocalFile(
        filesystem,
        sourceFilePath,
        tempDir
      )

      const isVideoInput = await hasVideoStream(sourceAbsPath).catch(
        () => false
      )
      ...
      if (!isVideoInput) {
        return {
          ...
          burnMessage:
            "Subtitles generated. Input has no video stream, so burn-in was skipped.",
        }
      }
      ...
      const baseColor = normalizeHexColor(
        resumeData.baseColor,
        DEFAULT_BASE_COLOR
      )
      const highlightColor = normalizeHexColor(
        resumeData.highlightColor,
      ...
      const animationPreset =
        resumeData.animationPreset ?? DEFAULT_SUBTITLE_ANIMATION_PRESET
      const safeAreaBottomPercent =
        resumeData.safeAreaBottomPercent ?? DEFAULT_SAFE_AREA_BOTTOM_PERCENT
...
export const burnSubtitlesStep = createStep({
  id: "burn-subtitles",
  description:
    "Burns generated subtitles into the video using FFmpeg subtitles filter.",
  ...
  execute: async ({ inputData, requestContext }) => {
    ...
    try {
      ...
      // Download files from S3 to local temp
      const { localPath: inputAbsPath } = await ensureLocalFile(
        filesystem,
        sourceFilePath,
        tempDir
      )
      const { localPath: assAbsPath } = await ensureLocalFile(
        filesystem,
        subtitleData.assPath,
        tempDir
      )
      ...
      const outputRelPath = path.join(
        "subtitle_file",
        `${sourceBase}_subtitled.mp4`
      )
      const outputAbsPath = path.join(tempDir, `${sourceBase}_subtitled.mp4`)
      const styledAssRelPath = path.join(
        "subtitle_file",
        `${sourceBase}_styled.ass`
      )
      const styledAssAbsPath = path.join(tempDir, `${sourceBase}_styled.ass`)
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        proc.on("close", (code) => {
          ...
          reject(
            new Error(
              `ffmpeg burn subtitles failed (code ${code}): ${stderr.get().slice(-1200)}`
            )
          )
        })
      ...
      await uploadToS3(filesystem, styledAssAbsPath, styledAssRelPath)
      await uploadToS3(filesystem, outputAbsPath, outputRelPath)
