...
function applySegmentCuts(
  ...
  return new Promise((resolve, reject) => {
    ...
    for (const silence of paddedSilences) {
      if (silence.start > cursor + 0.01) keep.push({ start: cursor, end: silence.start });
      cursor = silence.end;
    }
    if (cursor < totalDuration - 0.01) keep.push({ start: cursor, end: totalDuration });
    ...
    const suffix = isVideo ? `concat=n=${n}:v=1:a=1[vout][aout]` : `concat=n=${n}:v=0:a=1[aout]`;
    filterParts.push(`${concatInputs.join("")}${suffix}`);
    const mapArgs = isVideo ? ["-map", "[vout]", "-map", "[aout]"] : ["-map", "[aout]"];
    ...
    console.log("[applySegmentCuts] Command:", ffmpegInstaller.path, args.join(" "));
    ...
    proc.on("close", (code) => {
      ...
      if (code === 0) {
        ...
        console.error("[applySegmentCuts] ffmpeg error (code", code, "):", stderr.slice(-800));
...
const detectStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    // L10: sanitizePath is already called by the tool that starts this workflow;
    // we still call it here as a safety net but avoid the duplicate workspace join.
    const file = sanitizePath(inputData.file);
    const inputPath = path.join(WORKSPACE_PATH, file);

    console.log("[detectStep] Looking for file at:", inputPath);

    if (!fs.existsSync(inputPath)) {
      console.error("[detectStep] ERROR: File not found:", file);
      throw new Error(`File not found: ${file}`);
    }

    console.log("[detectStep] File found. Processing...");

    try {
      const [totalDuration, isVideo] = await Promise.all([
        getMediaDuration(inputPath),
        hasVideoStream(inputPath),
      ]);

      // Pass totalDuration so trailing silences are captured (H6)
      const segments = await detectSilences(inputPath, noiseThresholdDb, minSilenceDuration, totalDuration);
      ...
      const totalSilenceSeconds = parseFloat(
        segments.reduce((acc: number, s: SilenceSegment) => acc + s.duration, 0).toFixed(2),
      );

      const summary = [
        `File: ${file} (${isVideo ? "video" : "audio"})`,
        `Total duration: ${totalDuration.toFixed(1)}s`,
        `Silences found: ${segments.length}`,
        `Time to be removed: ${totalSilenceSeconds}s (${(totalSilenceSeconds / 60).toFixed(1)} min)`,
        ``,
        `Segments:`,
        ...segments.map((s: SilenceSegment, i: number) => `  [${i + 1}] ${s.start}s -> ${s.end}s (${s.duration}s)`),
      ].join("\n");
      ...
      console.error("[detectStep] ERROR during detection:", error.message);
      throw error;
    }
  },
});
...
const approvalStep = createStep({
  ...
  execute: async ({ inputData, resumeData, suspend }) => {
    ...
    if (!resumeData) {
      ...
      await suspend({
        message: "Review the detected silences and confirm whether to apply the cuts.",
      ...
      console.log("[approvalStep] WARNING: This code should not execute after suspend()");
      return {
        file: inputData.file,
    ...
    console.log("[approvalStep] Resuming with data:", JSON.stringify(resumeData, null, 2));
    return {
      file: inputData.file,
      fileType: inputData.fileType,
      totalDuration: inputData.totalDuration,
...
const cutStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    console.log("[cutStep] Input:", JSON.stringify({
      file: inputData.file,
      approved: inputData.approved,
      segmentsCount: inputData.segments.length,
    }, null, 2));

    const { file, fileType, totalDuration, segments, approved, preserveNaturalPauses } = inputData;
    ...
    if (!fs.existsSync(outputPath)) {
      console.error("[cutStep] ERROR: Output file was not generated:", outputRel);
      throw new Error(`Output file was not generated: ${outputRel}. Check that ffmpeg has write permissions.`);
    }
...
export const silenceCutterWorkflow = createWorkflow({
  id: "silence-cutter",
  description: "Detects silences in an audio/video file, pauses for human approval, and applies the cuts.",
  inputSchema: z.object({
    file: z.string().describe("Relative path within the workspace (e.g. wild_project.mp4)"),
